    ...
    for (int x = 0; x < this->loading_radius * 2 + 1; x++)
    {
        ...
        for (int y = 0; y < this->loading_radius * 2 + 1; y++) {
            ...
            for (int z = 0; z < this->loading_radius * 2 + 1; z++) {
                ...
                this->chunks[x][y][z].chunk_pos = Vector3Int(x, y, z) * (1<<this->chunk_depth);
            }
        }
    }
...
void World::load_circle(int radius) {
    ...
    Vector3Int offset = this->world_center + Vector3Int(1, 1, 1) * this->loading_radius;
...
Chunk& World::get_chunk(int x, int y, int z) {
    try
    {
        ...
        Chunk& chunk = this->chunks[x % (this->loading_radius * 2 + 1)][y % (this->loading_radius * 2 + 1)][z % (this->loading_radius * 2 + 1)];
        if (chunk.chunk_pos == Vector3Int(x, y, z)) return chunk;
        else throw std::exception();
    }
    catch(const std::exception& e)
    {
        std::cerr << "ERROR : wrong chunk index (" << x << ", " << y << ", " << z << ")\n";
        std::cerr << e.what() << "\n";
        exit(1);
    }
}
